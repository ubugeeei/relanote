...
use crate::types::{Type, TypeScheme, TyVar};
...
impl TypeContext {
    ...
    pub fn apply(&self, ty: &Type) -> Type {
        match ty {
            ...
            Type::Function(a, b) => {
                Type::function(self.apply(a), self.apply(b))
            }
            Type::Tuple(elems) => {
    ...
    pub fn instantiate(&mut self, scheme: &TypeScheme) -> Type {
        ...
        fn substitute(ty: &Type, subst: &HashMap<TyVar, Type>) -> Type {
            match ty {
                ...
                Type::Function(a, b) => {
                    Type::function(substitute(a, subst), substitute(b, subst))
                }
                Type::Tuple(elems) => {
    ...
    pub fn generalize(&self, ty: &Type) -> TypeScheme {
        ...
        let free_in_env: std::collections::HashSet<TyVar> = self
            .env
            .values()
            .flat_map(|s| s.ty.free_vars())
            .collect();
